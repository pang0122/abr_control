...
class OSC_force_lim(Controller):
    ...
    def __init__(
        ...
        print("modified version with a force threshold")
    ...
    def generate(
        ...
        # modify to increase so that external force is near limit (AI tool used)
        if external_force is not None and self.n_ctrlr_dof == 6 and self.force_lim is not None:
            f_ext = np.array(external_force[:3])
            f_norm = np.linalg.norm(f_ext)
            # Compute desired force vector to apply (in direction of external force)
            f_additional = 1.8*(self.force_lim - f_norm) * (f_ext / f_norm)
            f_push = np.concatenate((f_additional, np.zeros(3)))
            # Project into joint torques
            if np.linalg.norm(f_push) < 50: #prevent absolute chaos
                tau_push = J.T @ f_push
                u += tau_push
