...
class OSCVel(Controller):
    ...
    def __init__(
        ...
        correcting_force=None,
        dt=None
    ):
        print("modified version with ext force compensation")
        ...
        self.correcting_force = correcting_force
        self.gains = [1000,1,1]
        self.simple_pid_x = PID(output_limits=(-abs(self.correcting_force), abs(self.correcting_force)),
                                sample_time=dt,
                                Kp=self.gains[0],
                                Kd=self.gains[1],
                                Ki=self.gains[2],)
        self.simple_pid_y = PID(output_limits=(-abs(self.correcting_force), abs(self.correcting_force)),
                               sample_time=dt,
                                Kp=self.gains[0],
                                Kd=self.gains[1],
                                Ki=self.gains[2], )
    ...
    def generate(
        self, q, dq, target, target_velocity=None, ref_frame="EE", xyz_offset=None, #external_force=None,
    box_velocity=None, desired_box_velocity=None, override_task=False):
        ...
        box_velocity: list, optional (default:None)
            A 3 element vector of the current velocity of the object being pushed.
        desired_box_velocity: list, optional (default: None)
            a 3 element vector of the desired velocity of the object being pushed.
        override_task: bool, optional (default: False)
            Ignore the target and target_velocity, generate based off of other elements.
        """
        ...
        #speed comp:
        if box_velocity is not None and desired_box_velocity is not None and self.correcting_force is not None and override_task:
            #reduce the effect of moving towards the box
            #u*=0.8
            #simple PID only works on a single element

            velocity_diff = box_velocity - desired_box_velocity
            force_correction = np.zeros(6)
            force_correction[0] = self.simple_pid_x(velocity_diff[0])
            force_correction[1] = self.simple_pid_y(velocity_diff[1])
            force_correction[2] = 0
            correction = J.T @ np.hstack(force_correction)
            u += correction
...
'''
        ...
        if external_force is not None and self.n_ctrlr_dof == 6:
            #
            external_all = np.hstack((external_force, [0,0,0]))
            correction =J.T @ external_all
            u += correction
'''
